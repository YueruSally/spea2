    ...
    for scenario in SCENARIOS:
        ...
        for algo1_key, algo2_key, algo1_label, algo2_label in COMPARISONS:
            ...
            scenario_data.append({
                ...
                "U": U,
                "p_raw": p,
                "med_Algo1": np.median(vals1),
                "med_Algo2": np.median(vals2),
                "Algo1": algo1_label,
                "Algo2": algo2_label,
            })
        ...
        for i, row in enumerate(scenario_data):
            row["p_adj"] = p_adj[i]
            if metric == "HV":
                row["winner"] = row["Algo1"] if row["med_Algo1"] >= row["med_Algo2"] else row["Algo2"]
            else:
                row["winner"] = row["Algo1"] if row["med_Algo1"] <= row["med_Algo2"] else row["Algo2"]
...
def main():
    ...
    for metric in ["HV", "IGD+", "Spacing"]:
        ...
        for scenario in SCENARIOS:
            ...
            for algo_key, algo_label in algo_labels:
                ...
                row[algo_label] = round(np.median(vals), 4)
            medians = {al: row[al] for _, al in algo_labels}
            if metric == "HV":
                row["Best"] = max(medians, key=medians.get)
            else:
                row["Best"] = min(medians, key=medians.get)
            rows.append(row)
    ...
    for metric in ["HV", "IGD+", "Spacing"]:
        ...
        for opponent_label in ["NSGA-II", "MOEA/D"]:
            sub = ldf[ldf["Comparison"] == f"SPEA2_vs_{opponent_label}"]
            wins = ties = losses = 0

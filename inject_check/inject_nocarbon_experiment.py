...
# -*- coding: utf-8 -*-
"""
inject_nocarbon_experiment_v2.py
─────────────────────────────────────────────────────────────────────────────
在原版基础上，将 2 级固定支配链追踪升级为「无限递归淘汰链」追踪：
...
每一个被注入点或其支配者消失时，自动将新出现的支配者加入追踪池，
直到追踪链末端节点进入最终 Pareto 或被密度裁剪。

输出：inject_results_v2/
    injection_report_v2.txt   完整多级淘汰链报告
    survival_log.csv          每代存活状态
    survival_plot.png         存活曲线图
    chain_summary.txt         每个 nocarbon 点的淘汰链摘要（最易读）
─────────────────────────────────────────────────────────────────────────────
"""

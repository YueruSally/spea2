...
SPEA2 ── BASELINE (3-objective: Cost, Emission, Time)  30 runs
  ...
  path-lib: Martins multi-objective shortest-path (3-obj Pareto labeling)

[方案 B2] 将原 DFS 随机采样 + top-k 筛选替换为
Martins 多目标最短路算法,保证路径库是 (cost, emission, time)
三维 Pareto 最优路径集。其余所有 GA/SPEA2 逻辑不变。
...
# ── Path library (legacy DFS params, 仅在 USE_MARTINS=False 时使用) ──
PATHS_TOPK_PER_CRITERION = 20
PATH_LIB_CAP_TOTAL       = 60
DFS_MAX_PATHS_PER_OD     = 200

# ── Martins 多目标最短路算法参数 (方案 B2) ─────────────────
USE_MARTINS                = True      # True=使用 Martins, False=使用原 DFS
MARTINS_MAX_NODES_IN_PATH  = 12        # 路径最大节点数(与原 max_len=12 一致)
MARTINS_MAX_LABELS_PER_NODE = 500      # 每个节点保留的最多非支配标签数(安全阀)
MARTINS_MAX_TOTAL_LABELS   = 200000    # 单个 OD 的总标签数上限(安全阀)
MARTINS_CAP_PER_OD         = None      # 每个 OD 保留的最多路径数(None=不截断)
...
def build_path_library_dfs_legacy(node_names, node_region, arcs, batches, tt_dict, arc_lookup):
    """原版 DFS 随机采样 + top-k 筛选的路径库构造(保留作为对照)"""
...
# ════════════════════════════════════════════════════════
# Martins 多目标最短路算法 (方案 B2)
# ════════════════════════════════════════════════════════

@dataclass(order=True)
class _MartinsLabel:
    """
    Martins 算法中的标签 = 从起点到当前节点的一条部分路径。
    sort_key 放在最前且 compare=True 用于 heap 排序;其他字段不参与比较。
    """
...
def _dominates(a: tuple, b: tuple, eps: float = 1e-9) -> bool:
    """a 支配 b: 所有维度 a <= b, 且至少一维 a < b"""
    return (all(x <= y + eps for x, y in zip(a, b)) and
            any(x <  y - eps for x, y in zip(a, b)))


def _is_dominated_by_any(target: tuple, labels: list, eps: float = 1e-9) -> bool:
    """target 是否被 labels 中任一标签支配"""
    for L in labels:
        if _dominates(L.objectives, target, eps):
            return True
    return False


def martins_pareto_paths(
    graph: Dict[str, List[Tuple[str, object]]],
    origin: str,
    destination: str,
    node_region: Dict[str, str],
    max_nodes_in_path: int = 12,
    max_labels_per_node: int = 500,
    max_total_labels: int = 200000,
    verbose: bool = False,
) -> List[_MartinsLabel]:
    """
    计算 origin → destination 的所有 Pareto 最优路径 (3 目标: cost, emission, time)。

    算法:标签校正法 (label-correcting) + 优先队列 + 支配剪枝。
    约束: china_border_monotone_ok + region_monotone_ok (沿用原 baseline 的走廊约束)。

    返回: destination 处的非支配标签列表,每个标签包含完整路径。
    """
    # 每个节点维护一个"非支配标签列表"
    node_labels: Dict[str, List[_MartinsLabel]] = {origin: []}

    # 初始标签:起点,三目标全 0
    init_label = _MartinsLabel(
        sort_key=(0.0, 0.0, 0.0),
        cost=0.0, emission=0.0, time_h=0.0,
        node=origin,
        nodes=(origin,),
        arcs=(),
    )
    ...
    # 优先队列(heap),按三目标字典序处理"看起来更优"的标签
    heap: List[_MartinsLabel] = [init_label]
    ...
    while heap:
        ...
        for (v, arc) in graph.get(L.node, []):
            ...
            # 剪枝 1: 被 v 的已有标签支配 → 丢弃
            v_existing = node_labels.get(v, [])
            if _is_dominated_by_any(new_obj, v_existing):
                continue

            # 剪枝 2: 被终点的已有标签支配 → 丢弃(全局剪枝,关键加速)
            dest_existing = node_labels.get(destination, [])
            if dest_existing and _is_dominated_by_any(new_obj, dest_existing):
                continue

            # 构造新标签
            new_label = _MartinsLabel(
                sort_key=new_obj,
                cost=new_cost, emission=new_emis, time_h=new_time,
                node=v,
                nodes=tuple(new_nodes_list),
                arcs=L.arcs + (arc,),
            )

            # 从 v 的标签集中剔除被新标签支配的旧标签
            if v in node_labels:
                node_labels[v] = [L_old for L_old in node_labels[v]
                                  if not _dominates(new_obj, L_old.objectives)]
            else:
                node_labels[v] = []
            ...
            if total_labels_generated > max_total_labels:
                if verbose:
                    print(f"    [Martins] {origin}->{destination}: "
                          f"标签数超过 {max_total_labels},提前终止")
                heap.clear()
                aborted = True
                break
...
def build_path_library_martins(node_names, node_region, arcs, batches,
                                tt_dict, arc_lookup):
    """
    方案 B2: 用 Martins 多目标最短路算法构建 Pareto 最优路径库。
    返回结构与原 build_path_library 完全一致。
    """
    graph    = build_graph(arcs)
    path_lib = {}
    next_pid = 0
    ...
    print(f"[Martins] 开始为 {len(od_pairs)} 个 OD 对计算 Pareto 最优路径集...")
    ...
    for od_idx, (origin, dest) in enumerate(od_pairs):
        ...
        # 可选:每 OD 路径数截断(按 cost 均匀采样,保持多样性)
        ...
        print(f"  [{od_idx+1:2d}/{len(od_pairs)}] {origin} -> {dest}: "
              f"{len(pareto_labels):3d} pareto labels -> {len(paths_od):3d} paths "
              f"({time.perf_counter()-t_od:.2f}s)")

    elapsed = time.perf_counter() - t0
    print(f"[Martins] 完成。共 {len(path_lib)} 个 OD, {total_paths} 条路径, "
          f"平均 {total_paths/max(1,len(path_lib)):.1f} 条/OD, "
          f"耗时 {elapsed:.2f}s")
    return path_lib


# ════════════════════════════════════════════════════════
# 路径库统一入口:根据 USE_MARTINS 选择构造方式
# ════════════════════════════════════════════════════════

def build_path_library(node_names, node_region, arcs, batches, tt_dict, arc_lookup):
    """
    根据全局开关 USE_MARTINS 选择路径库构造方式:
      True  → Martins 多目标最短路算法 (方案 B2, 默认)
      False → 原 DFS 随机采样 + topk 筛选 (legacy)
    """
    if USE_MARTINS:
        print("[INFO] 使用 Martins 多目标最短路算法构建路径库 (方案 B2)")
        return build_path_library_martins(
            node_names, node_region, arcs, batches, tt_dict, arc_lookup)
    else:
        print("[INFO] 使用原 DFS 随机采样 + topk 筛选构建路径库 (legacy)")
        return build_path_library_dfs_legacy(
            node_names, node_region, arcs, batches, tt_dict, arc_lookup)

...
HV_EVERY     = 5
HV_SAMPLES   = 50000  # [④] 与Baseline一致（实际2D精确sweep不用此值，但保持统一）
METRIC_EVERY = 5
...
# ── Martins 路径库参数 ──────────────────────────────────
MARTINS_MAX_NODES_IN_PATH   = 12
MARTINS_MAX_LABELS_PER_NODE = 500
MARTINS_MAX_TOTAL_LABELS    = 200000
MARTINS_CAP_PER_OD          = None


# ════════════════════════════════════════════════════════
# Corridor helpers (unchanged)
# ════════════════════════════════════════════════════════
...
class _MartinsLabel:
    ...
    emission: float = field(compare=False, default=0.0)
    time_h:   float = field(compare=False, default=0.0)
    node:     str   = field(compare=False, default="")
    nodes:    tuple = field(compare=False, default=())
    arcs:     tuple = field(compare=False, default=())

    @property
    def objectives(self) -> tuple:
        return (self.cost, self.emission, self.time_h)


def _dominates_martins(a: tuple, b: tuple, eps: float = 1e-9) -> bool:
    return (all(x <= y + eps for x, y in zip(a, b)) and
            any(x <  y - eps for x, y in zip(a, b)))


def _is_dominated_by_any_martins(target: tuple, labels: list, eps: float = 1e-9) -> bool:
    for L in labels:
        if _dominates_martins(L.objectives, target, eps):
            return True
    return False


def martins_pareto_paths(graph, origin, destination, node_region,
                          max_nodes_in_path=12, max_labels_per_node=500,
                          max_total_labels=200000, verbose=False):
    node_labels = {origin: []}
    init_label = _MartinsLabel(
        sort_key=(0.0, 0.0, 0.0), cost=0.0, emission=0.0, time_h=0.0,
        node=origin, nodes=(origin,), arcs=(),
    )
    ...
    while heap:
        ...
        if L not in node_labels.get(L.node, []):
            continue
        if len(L.nodes) >= max_nodes_in_path:
            continue

        for (v, arc) in graph.get(L.node, []):
            if v in L.nodes:
                continue
            new_nodes_list = list(L.nodes) + [v]
            if not china_border_monotone_ok(new_nodes_list, node_region):
                continue
            if not region_monotone_ok(new_nodes_list, node_region):
                continue

            arc_cost = arc.cost_per_teu_km     * arc.distance
            arc_emis = arc.emission_per_teu_km * arc.distance
            arc_time = arc.distance / max(arc.speed_kmh, 1.0)
            new_cost = L.cost     + arc_cost
            new_emis = L.emission + arc_emis
            new_time = L.time_h   + arc_time
            new_obj  = (new_cost, new_emis, new_time)

            if _is_dominated_by_any_martins(new_obj, node_labels.get(v, [])):
                continue
            dest_existing = node_labels.get(destination, [])
            if dest_existing and _is_dominated_by_any_martins(new_obj, dest_existing):
                continue

            new_label = _MartinsLabel(
                sort_key=new_obj, cost=new_cost, emission=new_emis, time_h=new_time,
                node=v, nodes=tuple(new_nodes_list), arcs=L.arcs + (arc,),
            )

            if v in node_labels:
                node_labels[v] = [L_old for L_old in node_labels[v]
                                  if not _dominates_martins(new_obj, L_old.objectives)]
            else:
                node_labels[v] = []

            node_labels[v].append(new_label)
            heapq.heappush(heap, new_label)
            total_labels_generated += 1

            if len(node_labels[v]) > max_labels_per_node:
                node_labels[v].sort(key=lambda x: x.sort_key)
                node_labels[v] = node_labels[v][:max_labels_per_node]

            if total_labels_generated > max_total_labels:
                if verbose:
                    print(f"    [Martins] {origin}->{destination}: 标签数超限，提前终止")
                heap.clear()
                aborted = True
                break
    ...
    dest_labels = node_labels.get(destination, [])
    final = []
    for L in dest_labels:
        others_obj = [x.objectives for x in dest_labels if x is not L]
        if not any(_dominates_martins(o, L.objectives) for o in others_obj):
            final.append(L)
    return final


def build_path_library_martins(node_names, node_region, arcs, batches,
                                tt_dict, arc_lookup):
    graph    = build_graph(arcs)
    path_lib = {}
    next_pid = 0

    od_pairs, seen_od = [], set()
    for b in batches:
    ...
    print(f"[Martins] 开始为 {len(od_pairs)} 个OD对计算Pareto最优路径...")
    t0 = time.perf_counter()
    total_paths = 0

    for od_idx, (origin, dest) in enumerate(od_pairs):
        t_od = time.perf_counter()
        pareto_labels = martins_pareto_paths(
            graph=graph, origin=origin, destination=dest,
            node_region=node_region,
            max_nodes_in_path   = MARTINS_MAX_NODES_IN_PATH,
            max_labels_per_node = MARTINS_MAX_LABELS_PER_NODE,
            max_total_labels    = MARTINS_MAX_TOTAL_LABELS,
            verbose=True,
        )

        paths_od = []
        for L in pareto_labels:
            # [NoCarbon] 使用repair（有timetable约束）
            repaired = repair_arc_seq_with_road_fallback(list(L.arcs), tt_dict, arc_lookup)
            if repaired is None:
                continue
            ...
            p = Path(
                path_id=next_pid, origin=origin, destination=dest,
                nodes=nodes, modes=modes, arcs=repaired,
                base_cost_per_teu    =sum(a.cost_per_teu_km * a.distance for a in repaired),
                # ★ [NoCarbon] 无 base_emission_per_teu 字段
                base_travel_time_h   =sum(a.distance / max(a.speed_kmh, 1.0) for a in repaired),
            )
            paths_od.append(p)
            next_pid += 1

        if MARTINS_CAP_PER_OD and len(paths_od) > MARTINS_CAP_PER_OD:
            paths_od.sort(key=lambda p: p.base_cost_per_teu)
            step = max(1, len(paths_od) // MARTINS_CAP_PER_OD)
            paths_od = paths_od[::step][:MARTINS_CAP_PER_OD]

        if paths_od:
            path_lib[(origin, dest)] = paths_od
            total_paths += len(paths_od)

        print(f"  [{od_idx+1:2d}/{len(od_pairs)}] {origin} -> {dest}: "
              f"{len(pareto_labels):3d} labels -> {len(paths_od):3d} paths "
              f"({time.perf_counter()-t_od:.2f}s)")

    print(f"[Martins] 完成。{len(path_lib)} OD, {total_paths} 路径, "
          f"耗时 {time.perf_counter()-t0:.2f}s")
    return path_lib

def build_path_library(node_names, node_region, arcs, batches, tt_dict, arc_lookup):
    print("[INFO] 使用 Martins 多目标最短路算法构建路径库")
    return build_path_library_martins(
...
# ════════════════════════════════════════════════════════
# Simulation & evaluation (unchanged, no emission)
# ════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════
# [①] Fixed equal-weight operator selection (replaces adaptive roulette)
# ════════════════════════════════════════════════════════

def sample_operator() -> str:
    """Fixed equal-weight: each op has 0.25 probability."""
...
def mutate_fixed(
    ...
    """[①] Fixed equal-weight mutation — mirrors Baseline Doc9 mutate_fixed."""
...
# ════════════════════════════════════════════════════════
# Dominance (2-objective, unchanged)
# ════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════
# [②③] Feasibility Boost — uses mutate_fixed (no roulette)
# ════════════════════════════════════════════════════════
...
def feasibility_boost(archive, batches, path_lib, tt_dict, arc_lookup,
    ...
    """[②③] Uses mutate_fixed (fixed 0.25) instead of adaptive roulette.
    No roulette object required. evaluate only once per child."""
    ...
    if len(parents) < 2:
        ...
        for _ in range(boost_rounds):
            ...
            for child in (c1, c2):
                ...
                # [③] evaluate once before optional mutation
                evaluate_individual(child, batches, arcs, tt_dict,
                                    waiting_cost_per_teu_h, **eval_kwargs)
                if random.random() < boost_mutation_rate:
                    # [①] mutate_fixed — evaluate internally only if ok=True
                    mutate_fixed(
...
# ════════════════════════════════════════════════════════
# Plotting (updated: [⑤] fixed op prob title)
# ════════════════════════════════════════════════════════
...
def plot_operator_prob(gen, save="plot_OperatorProb.png"):
    """[⑤] Fixed equal-weight — shows constant 0.25 lines."""
...
# ════════════════════════════════════════════════════════
# SPEA2 Runner — [①②③] fixed mutation, no roulette
# ════════════════════════════════════════════════════════
...
def run_spea2_nocarbon(filename="data.xlsx", pop_size=125, generations=400,
                        shared_data=None):
    """[①] No roulette object — uses mutate_fixed throughout."""
    ...
    for gen in range(generations):
        ...
        # Step 3: Feasibility boost [②③ uses mutate_fixed]
        ...
        # Step 5: Crossover + Mutation [①] uses mutate_fixed
        offspring = []
        while len(offspring) < pop_size:
            ...
            # [①] mutate_fixed (fixed 0.25) — evaluates internally if ok=True
            ...
            # Mandatory single evaluate per child (mirrors Baseline Doc9)
...
if __name__ == "__main__":
    ...
    plot_operator_prob(gen)   # [⑤] fixed 0.25 lines
